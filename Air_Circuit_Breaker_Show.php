...
<body>
    ...
    <script>
        ...
        function saveInspectionData() {
            ...
            // جمع البيانات من جدول الفحص (الجدول الرابع)
            ...
            rowsToSave.forEach(row => {
                // جمع قيمة SR من خاصية البيانات
                const sr = row.getAttribute('data-sr');
                srs.push(sr);

                // جمع بيانات "جيد" من الخلايا
                const goodCells = row.querySelectorAll('td.good-cell');
                goodCells.forEach(cell => {
                    const stateValue = cell.textContent.trim();
                    states.push(stateValue);
                });
            });

            // تجميع كل البيانات في كائن واحد لإرساله
            const postData = {
                states: states,
                srs: srs
            };

            // إرسال البيانات إلى save_data.php باستخدام fetch
            fetch('save_data.php', {
                    method: 'POST',
                    headers: {
    ...
    <table id="inspection-table">
        ...
        <tr>
            ...
            <tr>
                ...
                <td class='right-align' colspan='6'></td>
            </tr>
            <tr>
